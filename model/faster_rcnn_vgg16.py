...
def decom_vgg16():
    ...
    print('len model.feature',len(list(model.features)))
...
class FasterRCNNVGG16Extractor(FasterRCNN):
    def __init__(self,faster_rcnn_vgg16,n_class,hidden_size
                 ):

        rpn = faster_rcnn_vgg16.rpn

        head = VGG16RoIExtractorHead(
            n_class=n_class,
            head=faster_rcnn_vgg16.head,
            hidden_size=hidden_size
        )

        super(FasterRCNNVGG16Extractor, self).__init__(
            faster_rcnn_vgg16.extractor,
            rpn,
            head,
        )


class VGG16RoIExtractorHead(nn.Module):

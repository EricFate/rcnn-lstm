...
class FasterRCNN(nn.Module):
    ...
    def hidden_size(self):
        return self.head.score.in_features
    ...
    def extract(self, x, num_box):
        ...
        for i, (img, size) in enumerate(zip(prepared_imgs, sizes)):
            ...
            roi_cls_loc_, roi_score_, feature = self.head(
                h, roi, roi_indices)
            ...
            bbox, label, score, feat = self._suppress_by_num(raw_cls_bbox, raw_prob, feature, num_box)
            features[i, :, :] = feat
    ...
    def _suppress_by_num(self, raw_cls_bbox, raw_prob, feature, num_box):
        bbox = list()
        label = list()
        score = list()
        feat = list()
        for l in range(1, self.n_class):
            ...
            feat.append(feature[keep])
        bbox = np.concatenate(bbox, axis=0).astype(np.float32)
        label = np.concatenate(label, axis=0).astype(np.int32)
        score = np.concatenate(score, axis=0).astype(np.float32)
        feat = t.cat(feat, dim=0)
        args = np.argsort(score)[:num_box]
        return bbox[args], label[args], score[args], feat[args]
